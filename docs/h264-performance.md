...
| --- | --- | ---: | ---: | ---: | ---: |
| decv Serial | NV12 | 3.63 s | 3.54 s | 84,764 KiB | 49.6 FPS |
| decv Auto (2 workers) | NV12 | 3.62 s | 3.99 s | 84,484 KiB | 49.7 FPS |
| FFmpeg 1 thread | NV12 | 0.62 s | 0.72 s | 152,292 KiB | 290.3 FPS |
| FFmpeg Auto | NV12 | 0.26 s | 1.42 s | 293,668 KiB | 692.3 FPS |
| FFmpeg 1 thread | decode-only | 0.57 s | 0.55 s | 95,608 KiB | 315.8 FPS |
| FFmpeg Auto | decode-only | 0.21 s | 0.95 s | 189,436 KiB | 857.1 FPS |

On this workload:

- decv Serial takes about **5.9x** as much wall time as single-threaded FFmpeg
  when both produce NV12;
- decv Auto takes about **13.9x** as much wall time as FFmpeg Auto when both
  produce NV12;
...
seconds. The current 3.63-second Serial result is about 0.83x real time, or
roughly 21% more wall-clock work than the target permits.
...
seconds. Public low-level CABAC APIs retain their transactional rollback
semantics.
